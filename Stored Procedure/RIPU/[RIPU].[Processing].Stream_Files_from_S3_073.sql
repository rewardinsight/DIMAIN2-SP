USE [RIPU]
GO
/****** Object:  StoredProcedure [Processing].[Stream_Files_from_S3_073]    Script Date: 19/08/2026 10:09:25 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [Processing].[Stream_Files_from_S3_073]
(
...
BEGIN
    ...
    IF CHARINDEX(':', @Directory) = 0
        SET @Directory = 'E:\DataOpsFunctions\RIRUPipelne\reward-dataops-ingest-to-dimain-from-s3-073'

    SET @Directory = REPLACE(@Directory, ' ', '" "')

    SET @sql = 'python "E:\DataOpsFunctions\RIRUPipelne\reward-dataops-ingest-to-dimain-from-s3-073\run_injest_mosaic.py"'
