USE [RIPU]
GO
/****** Object:  StoredProcedure [Processing].[RIRUPipeline_StreamToS3_V2]    Script Date: 19/08/2026 10:03:44 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [Processing].[RIRUPipeline_StreamToS3_V2]
(
...
BEGIN
    ...
    IF CHARINDEX(':', @Directory) = 0
        SET @Directory = 'E:\DataOpsFunctions\RIRUPipelne\reward-dataops-dimain-s3-stream_V2'

    SET @Directory = REPLACE(@Directory, ' ', '" "')

    SET @sql = 'python "E:\DataOpsFunctions\RIRUPipelne\reward-dataops-dimain-s3-stream_V2\run_export.py"'
