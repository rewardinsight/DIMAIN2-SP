USE [RIPU]
GO
/****** Object:  StoredProcedure [Processing].[RIRUPipeline_StreamToS3_Parallel]    Script Date: 19/08/2026 10:02:35 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [Processing].[RIRUPipeline_StreamToS3_Parallel]
--(
...
BEGIN
    ...
    --IF CHARINDEX(':', @Directory) = 0
        --SET @Directory = 'E:\DataOpsFunctions\Parallel_Streamer_Scripts'

    --SET @Directory = REPLACE(@Directory, ' ', '" "')

    SET @sql = 'python "E:\DataOpsFunctions\Parallel_Streamer_Scripts\run_export_parallel.py"'
    ...
    BEGIN
        ...
        DECLARE @ErrorEnd VARCHAR(300) = '... SELECT * FROM logs.RIRUPipeline_Log WHERE RunID =' + CAST(@RunID AS varchar) + ' for more info'
    ...
    DECLARE @Msg VARCHAR(300) = 'SELECT * FROM logs.RIRUPipeline_Log WHERE RunID = ' + CAST(@RunID AS varchar) + ' for more info'
    RAISERROR(@Msg, 1, -1, -1)
END
